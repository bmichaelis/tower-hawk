...
public class NagiosSyntaxThresholdTest {
	...
	private void evaluateMultiple(Threshold threshold, Map<Integer, Status> expected) {
		expected.forEach((k, v) -> {
			...
			try {
				threshold.evaluate(builder, k.doubleValue(), false, false);
			} catch (Exception e) {
				e.printStackTrace();
			}
	...
	public void evaluateObject() throws Exception {
		Threshold threshold = new NagiosSyntaxThreshold("10", "20");
		threshold.evaluate(builder, new Long(5), false, false);
		assertTrue("Expected that a Number type succeeds on evaluation", builder.getStatus() == SUCCEEDED);
		threshold.evaluate(builder, new Object(), false, false);
